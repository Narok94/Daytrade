...
const DashboardPanel: React.FC<any> = ({ activeBrokerage, customEntryValue, setCustomEntryValue, customPayout, setCustomPayout, addRecord, deleteTrade, selectedDateString, setSelectedDate, dailyRecordForSelectedDay, startBalanceForSelectedDay, isDarkMode, dailyGoalTarget, records, usdRate }) => {
    ...
    // Reset ignore state if date changes or if no stops are active
    useEffect(() => {
        if (!stopWinReached && !stopLossReached) {
            setIsIgnoringStop(false);
        }
    }, [stopWinReached, stopLossReached, selectedDateString]);
                ...
                <div className={`p-10 rounded-[2.5rem] border ${theme.card} shadow-2xl relative overflow-hidden`}>
                    ...
                    <div className="space-y-10">
                        ...
                        <div className="grid grid-cols-2 gap-8 pt-4">
                            <button onClick={() => handleQuickAdd('win')} disabled={(stopWinReached || stopLossReached) && !isIgnoringStop} className="h-20 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black rounded-3xl uppercase text-xs tracking-[0.3em] transition-all shadow-2xl shadow-emerald-500/20 active:scale-95 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed border-b-4 border-emerald-700">WIN (COMPRA)</button>
                            <button onClick={() => handleQuickAdd('loss')} disabled={(stopWinReached || stopLossReached) && !isIgnoringStop} className="h-20 bg-red-600 hover:bg-red-500 text-white font-black rounded-3xl uppercase text-xs tracking-[0.3em] transition-all shadow-2xl shadow-red-500/20 active:scale-95 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed border-b-4 border-red-800">LOSS (VENDA)</button>
                        </div>
                        {(stopWinReached || stopLossReached) && (
                            <div className={`p-6 rounded-[2rem] border text-center animate-in fade-in slide-in-from-top-2 duration-500 ${stopWinReached ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
                                <p className="text-[11px] font-black uppercase tracking-[0.2em] mb-4">
                                    {stopWinReached ? 'STOP GAIN ATINGIDO! META DO DIA CONCLUÍDA.' : 'STOP LOSS ATINGIDO! PRESERVE SEU CAPITAL.'}
                                </p>
                                {!isIgnoringStop ? (
                                    <button 
                                        onClick={() => setIsIgnoringStop(confirm("Você atingiu seu limite operacional definido na gestão de risco. Tem certeza que deseja ignorar o stop e continuar operando? (Risco de quebra de banca)"))} 
                                        className="px-6 py-2.5 bg-white/10 hover:bg-white/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                                    >
                                        Continuar Operando?
                                    </button>
                                ) : (
                                    <p className="text-[9px] font-bold opacity-60 uppercase italic tracking-widest">Modo Override: Operando fora da gestão de risco</p>
                                )}
                            </div>
                        )}
                    </div>
