...
import { Brokerage, DailyRecord, AppRecord, Trade, User, Goal, AIAnalysisResult, AISignalFeedback, RegisteredUser, UserPermissions } from './types';
...
import { 
    ...
    XMarkIcon, UsersIcon, LockClosedIcon
        ...
        <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:justify-between items-start gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className={`text-2xl font-black ${theme.text}`}>Central Administrativa</h2>
                        <p className={theme.textMuted}>Gestão de operadores e níveis de acesso.</p>
                    </div>
                </div>
                <button onClick={() => setIsCreating(true)} className="px-6 py-3 rounded-xl bg-teal-500 text-slate-950 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-teal-500/20">Novo Operador</button>
            </div>
            ...
            {isCreating && (
                <div className={`p-6 rounded-3xl border ${theme.card} relative overflow-hidden`}>
                    ...
                    <h3 className="text-sm font-black uppercase mb-4">Cadastrar Novo Usuário</h3>
                    <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <button type="submit" className="h-12 bg-slate-900 text-white font-black rounded-xl uppercase text-[10px] tracking-widest border border-slate-800">Confirmar Cadastro</button>
                    </form>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                            <tr className={`text-[10px] uppercase font-black tracking-widest ${isDarkMode ? 'bg-slate-950/50' : 'bg-slate-100/50'}`}>
                                ...
                                <th className="py-5 px-6">Nível</th>
                                <th className="py-5 px-6 text-center">IA</th>
                                <th className="py-5 px-6 text-center">Juros</th>
                                <th className="py-5 px-6 text-center">Metas</th>
                                <th className="py-5 px-6 text-center">Relatórios</th>
                                ...
                                <tr key={u.id} className="text-sm font-bold hover:bg-slate-800/5 transition-colors">
                                    <td className="py-4 px-6">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] opacity-40 font-mono">ID #{u.id}</span>
                                        </div>
                                    </td>
                                    <td className="py-4 px-6">
                                    ...
                                    <td className="py-4 px-6 text-center">
                                        <PermissionToggle active={u.permissions?.ai_access} onClick={() => handleUpdatePermission(u, 'ai_access')} />
                                    </td>
                                    <td className="py-4 px-6 text-center">
                                        <PermissionToggle active={u.permissions?.compound_access} onClick={() => handleUpdatePermission(u, 'compound_access')} />
                                    </td>
                                    <td className="py-4 px-6 text-center">
                                        <PermissionToggle active={u.permissions?.goals_access} onClick={() => handleUpdatePermission(u, 'goals_access')} />
                                    </td>
                                    <td className="py-4 px-6 text-center">
                                        <PermissionToggle active={u.permissions?.reports_access} onClick={() => handleUpdatePermission(u, 'reports_access')} />
                                    </td>
                                    <td className="py-4 px-6 text-right">
                                        <button className="text-red-500/50 hover:text-red-500 transition-colors"><TrashIcon className="w-5 h-5"/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
