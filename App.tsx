...
const App: React.FC<{ user: User; onLogout: () => void }> = ({ user, onLogout }) => {
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            // Load from localStorage as primary source in DB-less mode
            const localData = localStorage.getItem(`app_data_${user.id}`);
            if (localData) {
            ...
            console.error(e);
            // Default setup if everything fails
...
const AdminPanel: React.FC<{ theme: any, adminId: string | number }> = ({ theme, adminId }) => {
    ...
    const handleResetPassword = async (targetUserId: string | number) => {
