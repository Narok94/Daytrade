...
// --- AI Analysis Panel (V3 - Anti-Bias) ---
...
const AIAnalysisPanel: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => {
            ...
            const response = await ai.models.generateContent({
                ...
                contents: {
                    parts: [
                        ...
                        { text: "Analise este gráfico de Opções Binárias. Identifique suportes e resistências. Não tenha viés de venda. Determine a força de Touros e Ursos de 0 a 100. Retorne JSON: {recommendation: 'COMPRA'|'VENDA'|'AGUARDAR', entry_time: string, timeframe: string, confidence: number, reasoning: string, bull_force: number, bear_force: number}" }
                    ]
                },
                config: {
                    ...
                    responseSchema: {
                        ...
                        required: ["recommendation", "entry_time", "timeframe", "confidence", "reasoning", "bull_force", "bear_force"]
                    }
                }
            });
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-6">
            <h2 className="text-3xl font-black uppercase italic text-emerald-500">Sniper IA Engine</h2>
                ...
                <div className={`p-6 rounded-[2.5rem] border ${theme.card} flex flex-col min-h-[400px]`}>
                    {!selectedImage ? (
                        <label className="flex-1 border-2 border-dashed border-slate-800 rounded-[2rem] flex flex-col items-center justify-center cursor-pointer hover:bg-emerald-500/5 transition-all">
                            ...
                            <span className="text-[10px] font-black uppercase opacity-40">Enviar Print do Gráfico</span>
                        ...
                        <div className="flex-1 flex flex-col gap-4">
                            ...
                            <button onClick={analyzeChart} disabled={isAnalyzing} className="h-14 bg-emerald-500 text-slate-950 font-black rounded-2xl uppercase tracking-widest flex items-center justify-center gap-2">
                            ...
                            <button onClick={() => {setSelectedImage(null); setResult(null);}} className="text-[10px] font-black text-red-500 uppercase">Remover Imagem</button>
                        </div>
                    )}
                </div>
                <div className={`p-8 rounded-[2.5rem] border ${theme.card}`}>
                    <h3 className="text-[10px] font-black uppercase text-blue-400 mb-6">Resultado da Análise</h3>
                    {result ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center opacity-20 py-20"><CpuChipIcon className="w-16 h-16 mb-2" /><p className="text-[10px] font-black uppercase">Aguardando Gráfico</p></div>
                    )}
                </div>
            </div>
        </div>
...
const CompoundInterestPanel: React.FC<{ isDarkMode: boolean, activeBrokerage: Brokerage, records: AppRecord[] }> = ({ isDarkMode, activeBrokerage, records }) => {
    ...
    const tableData = useMemo(() => {
        ...
        return Array.from({ length: 30 }, (_, i) => {
            ...
            if (actual) {
                ...
                profit = curr * 0.03; // Simulação de 3% (3 wins de 1%)
                curr += profit;
            }
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-6">
            <h2 className="text-2xl font-black uppercase tracking-tighter">Projeção 30 Dias (Meta 3% ao Dia)</h2>
            <div className={`rounded-[2rem] border ${theme.card} overflow-hidden`}>
                <table className="w-full text-left text-xs">
                    <thead className="bg-slate-900 font-black text-slate-500 uppercase tracking-widest"><tr className="border-b border-slate-800"><th className="p-5">DIA</th><th className="p-5">BANCA INICIAL</th><th className="p-5">LUCRO (3%)</th><th className="p-5">BANCA FINAL</th><th className="p-5">STATUS</th></tr></thead>
                    <tbody className="divide-y divide-slate-800">
                        {tableData.map(d => (
                            <tr key={d.day} className={`hover:bg-slate-900/40 transition-colors ${!d.isActual ? 'opacity-40 grayscale-[0.5]' : ''}`}>
                                <td className="p-5 font-black text-slate-400">{d.day}</td>
                                <td className="p-5 font-bold">R$ {formatMoney(d.start)}</td>
                                <td className="p-5 text-green-500 font-black">+R$ {formatMoney(d.profit)}</td>
                                <td className="p-5 font-black text-white">R$ {formatMoney(d.end)}</td>
                                <td className="p-5"><div className={`px-2 py-1 rounded-md text-[8px] font-black uppercase text-center inline-block ${d.isActual ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-slate-800 text-slate-500'}`}>{d.isActual ? 'Concluido' : 'Projetado'}</div></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
    ...
    return (
        <div className="p-8 space-y-8 max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div><h2 className={`text-3xl font-black ${theme.text} tracking-tighter uppercase italic`}>Dashboard Sniper</h2><p className={theme.textMuted}>Gestão de Banca e Risco</p></div>
                <input type="date" value={selectedDateString} onChange={e => setSelectedDate(new Date(e.target.value + 'T12:00:00'))} className={`p-3 rounded-xl border ${theme.input} font-black`} />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                <div className={`p-6 rounded-[2rem] border ${theme.card}`}><p className="text-[9px] uppercase font-black text-slate-500 mb-2">Banca Atual</p><p className="text-2xl font-black text-green-500">R$ {formatMoney(dailyRecordForSelectedDay?.endBalanceUSD ?? startBalanceForSelectedDay)}</p></div>
                ...
                <div className={`p-6 rounded-[2rem] border ${theme.card}`}><p className="text-[9px] uppercase font-black text-slate-500 mb-2">Placar Diário</p><p className="text-2xl font-black text-blue-400">{winCount}W - {lossCount}L</p></div>
                <div className={`p-6 rounded-[2rem] border ${theme.card}`}><p className="text-[9px] uppercase font-black text-slate-500 mb-2">Status de Risco</p><p className={`text-xs font-black uppercase ${stopReached ? 'text-red-500' : 'text-green-500'}`}>{stopReached ? 'Meta Batida / Stop Ativado' : 'Operando'} ({winCount}/{stopWin}W | {lossCount}/{stopLoss}L)</p></div>
            </div>

            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <div className={`p-8 rounded-[2.5rem] border ${theme.card} space-y-6`}>
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Lançamento de Ordem</h3>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase ml-1">Valor da Ordem</label><input type="number" value={customEntryValue} onChange={e => setCustomEntryValue(e.target.value)} className={`w-full h-12 px-4 rounded-xl border font-bold ${theme.input}`} /></div>
                    ...
                    <div className="grid grid-cols-2 gap-6 pt-2">
                        <button disabled={stopReached} onClick={() => addRecord(1, 0, parseFloat(customEntryValue), parseFloat(customPayout))} className="h-16 bg-green-500 text-slate-950 font-black rounded-2xl uppercase tracking-widest hover:scale-[1.02] active:scale-95 disabled:grayscale">VITÓRIA (WIN)</button>
                        <button disabled={stopReached} onClick={() => addRecord(0, 1, parseFloat(customEntryValue), parseFloat(customPayout))} className="h-16 bg-red-600 text-white font-black rounded-2xl uppercase tracking-widest hover:scale-[1.02] active:scale-95 disabled:grayscale">DERROTA (LOSS)</button>
                    </div>
                </div>
                <div className={`p-8 rounded-[2.5rem] border ${theme.card} flex flex-col max-h-[400px]`}>
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-6">Histórico Recente</h3>
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                        {dailyRecordForSelectedDay?.trades?.length ? [...dailyRecordForSelectedDay.trades].reverse().map(t => (
                            <div key={t.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-950/50 border border-slate-900">
...
const SettingsPanel: React.FC<any> = ({ brokerage, setBrokerages, isDarkMode }) => {
    ...
    return (
        <div className="p-8 max-w-2xl mx-auto space-y-6">
            <h2 className="text-2xl font-black uppercase">Gestão Operacional</h2>
            <div className={`p-10 rounded-[3rem] border ${theme.card} space-y-8`}>
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-500">Nome da Estratégia</label><input type="text" value={name} onChange={e => setName(e.target.value)} className={`w-full h-12 px-4 rounded-xl border ${theme.input} font-black`} /></div>
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-500">Banca Inicial (R$)</label><input type="number" value={bal} onChange={e => setBal(e.target.value)} className={`w-full h-12 px-4 rounded-xl border ${theme.input} font-black`} /></div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-500">Valor/Porcentagem</label><input type="number" value={entryValue} onChange={e => setEntryValue(e.target.value)} className={`w-full h-12 px-4 rounded-xl border ${theme.input} font-black`} /></div>
                </div>
                <div className="grid grid-cols-2 gap-6">
                ...
                <button onClick={handleSave} className="w-full h-16 bg-emerald-500 text-slate-950 font-black rounded-2xl uppercase tracking-widest shadow-xl shadow-emerald-500/10">Salvar Parâmetros</button>
            </div>
        </div>
    );
};
...
const App: React.FC<{ user: User; onLogout: () => void }> = ({ user, onLogout }) => {
    ...
    const recalibrate = (all: AppRecord[], init: number) => {
        ...
        return all.sort((a,b) => a.id.localeCompare(b.id)).map(r => {
            ...
            const profit = daily.trades.reduce((acc, t) => acc + (t.result === 'win' ? t.entryValue * (t.payoutPercentage/100) : -t.entryValue), 0);
    ...
    const menu = [
        ...
        { id: 'settings', label: 'Gestão', icon: SettingsIcon },
    ];

    return (
        <div className={`flex h-screen overflow-hidden ${theme.bg} ${theme.text}`}>
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r ${theme.sidebar} transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
                <div className="h-20 flex items-center px-8 font-black italic text-emerald-400 text-xl tracking-tighter">HRK SNIPER</div>
                ...
                <div className="p-4 border-t border-slate-800"><button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 text-red-500 font-black hover:bg-red-500/10 rounded-2xl"><LogoutIcon className="w-5 h-5" />Sair</button></div>
            </aside>
            <main className="flex-1 flex flex-col overflow-hidden">
                <header className={`h-20 flex items-center justify-between px-6 border-b ${theme.header} z-30`}>
                    <div className="flex items-center gap-4"><button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden"><MenuIcon className="w-7 h-7" /></button><SavingStatusIndicator status={savingStatus} /></div>
                    <div className="flex items-center gap-3"><button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white transition-all shadow-sm">{isDarkMode ? <SunIcon className="w-5 h-5" /> : <MoonIcon className="w-5 h-5" />}</button><div className="w-10 h-10 rounded-2xl bg-emerald-500 flex items-center justify-center text-slate-950 font-black text-xs">{user.username.slice(0, 2).toUpperCase()}</div></div>
                </header>
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {activeTab === 'dashboard' && <DashboardPanel activeBrokerage={activeBrokerage} customEntryValue={customEntryValue} setCustomEntryValue={setCustomEntryValue} customPayout={customPayout} setCustomPayout={setCustomPayout} addRecord={addRecord} deleteTrade={deleteTrade} selectedDateString={selectedDate.toISOString().split('T')[0]} setSelectedDate={setSelectedDate} dailyRecordForSelectedDay={dailyRecordForSelectedDay} startBalanceForSelectedDay={activeBrokerage.initialBalance} isDarkMode={isDarkMode} />}
                    ...
                    {activeTab === 'report' && <div className="p-8 max-w-5xl mx-auto space-y-4"><h2 className="text-2xl font-black uppercase">Histórico Geral</h2>{records.flatMap(r => (r as DailyRecord).trades.map(t=>({...t, date: r.id}))).sort((a,b)=>b.timestamp-a.timestamp).map(t=>(<div key={t.id} className={`p-4 rounded-2xl border ${theme.card} flex justify-between items-center`}><div className="flex items-center gap-4"><div className={`w-1 h-10 rounded-full ${t.result === 'win' ? 'bg-green-500' : 'bg-red-500'}`} /><div><p className="text-[10px] font-black text-slate-500">{t.date}</p><p className="font-bold">{t.result === 'win' ? 'Vitória Sniper' : 'Loss Calculado'}</p></div></div><p className={`font-black ${t.result === 'win' ? 'text-green-500' : 'text-red-500'}`}>R$ {formatMoney(t.result === 'win' ? t.entryValue * (t.payoutPercentage/100) : t.entryValue)}</p></div>))}</div>}
