...
const AIAnalysisPanel: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => {
            ...
            const response = await ai.models.generateContent({
                ...
                contents: {
                    parts: [
                        ...
                        { text: "Analise este gráfico de Opções Binárias sem viés. Identifique suportes (Compra) e resistências (Venda). Retorne JSON: {recommendation: 'COMPRA'|'VENDA'|'AGUARDAR', confidence: number, reasoning: string, bull_force: number, bear_force: number}" }
                    ]
                },
                config: {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto space-y-6">
            <h2 className="text-3xl font-black uppercase italic text-emerald-500">Sniper IA Engine V2</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className={`p-8 rounded-[2.5rem] border ${theme.card}`}>
                    <h3 className="text-[10px] font-black uppercase text-blue-400 mb-6">IA Analytics</h3>
                    {result ? (
                        <div className="space-y-6">
                            <div className="flex justify-between items-end border-b border-slate-800 pb-6">
                                ...
                                <div className="text-right"><p className="text-[10px] uppercase font-black text-slate-500 mb-1">Confiança</p><p className="text-3xl font-black text-white">{result.confidence}%</p></div>
                            </div>
                    ...
                    <tbody className="divide-y divide-slate-800">
                        {tableData.map(d => (
                            <tr key={d.day} className={`transition-colors ${!d.isActual ? 'opacity-30 grayscale' : 'bg-emerald-500/5'}`}>
                                ...
                                <td className="p-5"><span className={`px-2 py-1 rounded-md text-[8px] font-black uppercase ${d.isActual ? 'bg-green-500/20 text-green-500' : 'bg-slate-800 text-slate-500'}`}>{d.isActual ? 'Real' : 'Projeção'}</span></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
    ...
    return (
        <div className="p-8 space-y-8 max-w-7xl mx-auto">
            <div className="flex justify-between items-center">
                <div><h2 className={`text-3xl font-black ${theme.text} tracking-tighter uppercase italic`}>Dashboard Sniper</h2><p className={theme.textMuted}>Gestão Profissional de Daytrade</p></div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                <div className={`p-8 rounded-[3rem] border ${theme.card} space-y-6 relative overflow-hidden`}>
                    ...
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Painel de Operação</h3>
                    ...
                    <div className="grid grid-cols-2 gap-6 pt-2">
                        <button disabled={stopReached} onClick={() => addRecord(1, 0, parseFloat(customEntryValue), parseFloat(customPayout))} className="h-16 bg-green-500 text-slate-950 font-black rounded-2xl uppercase tracking-widest hover:scale-[1.02] active:scale-95 disabled:grayscale disabled:opacity-50 shadow-lg shadow-green-500/20">WIN</button>
                        <button disabled={stopReached} onClick={() => addRecord(0, 1, parseFloat(customEntryValue), parseFloat(customPayout))} className="h-16 bg-red-600 text-white font-black rounded-2xl uppercase tracking-widest hover:scale-[1.02] active:scale-95 disabled:grayscale disabled:opacity-50 shadow-lg shadow-red-500/20">LOSS</button>
                    </div>
                </div>
